...
function Post( { isJob } ) {
    ...
    return (
        <div className='post-grid-container bg-primarylight flex flex-col items-center gap-[1rem]  '>
            <p className='text-secondary ' >{ isJob ? 'หางาน' : 'จ้างงาน' }</p>
            {filterData.map((items) =>
                <PostObject key={items.postID} postID={items.postID} postDate={items.postDate} title={items.title} detail={items.details} price={items.salary} profilePic={items.members.profile_picture} fname={items.members.fname} lname={items.members.lname} category={items.category.name} location={items.location} rating={items.review} images={items.picture}  posterID={items.posterID} isJob={items.isJob} />
            )}
        </div>
